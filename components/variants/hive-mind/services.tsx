...
const services = [
  {
    ...
    description:
      "We design and build scalable digital products from concept to production. Our engineering approach focuses on performance, reliability, and maintainability.",
...
function TiltCard({
  ...
  return (
    <div
      ...
      className={`tilt-card group relative flex h-[360px] flex-col overflow-hidden rounded-lg border border-border bg-card p-8 transition-all duration-700 hover:border-primary/40 ${
              ...
              <div key={service.title} data-index={i}>
                <TiltCard index={i} isVisible={visibleCards.has(i)}>
                  ...
                  >
                    ...
                    <p className="mt-3 text-sm leading-relaxed text-muted-foreground line-clamp-3 h-[66px]">
                      {service.description}
                    </p>

                    <div className="mt-5 grid grid-cols-3 gap-2 min-h-[56px]">
                      {service.tags.map((tag) => (
                        <span
                          key={tag}
                          className="rounded-full bg-secondary px-3 py-1 text-xs whitespace-nowrap text-muted-foreground transition-colors group-hover:bg-primary/10 group-hover:text-primary/80"
                        >
                          {tag}
                        </span>
                      ))}
